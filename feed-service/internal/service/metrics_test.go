...
func TestObserveBusinessOperationRecordsFeedMetric(t *testing.T) {
	counter := businessOperationTotal.WithLabelValues("feed-service", "get_feed", businessStatusSuccess)
	before := counterValue(t, counter)

	observeBusinessOperation("get_feed", time.Now().Add(-time.Millisecond), businessStatusSuccess)
