...
func observeBusinessOperation(operation string, started time.Time, status string) {
	businessOperationDuration.WithLabelValues("feed-service", operation).Observe(time.Since(started).Seconds())
	businessOperationTotal.WithLabelValues("feed-service", operation, status).Inc()
}
