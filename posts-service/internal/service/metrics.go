...
func observeBusinessOperation(operation string, started time.Time, status string) {
	businessOperationDuration.WithLabelValues("posts-service", operation).Observe(time.Since(started).Seconds())
	businessOperationTotal.WithLabelValues("posts-service", operation, status).Inc()
}
