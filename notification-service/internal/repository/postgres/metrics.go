...
func observeDBOperation(operation string, started time.Time, err error) {
	dbQueryDuration.WithLabelValues("notification-service", operation).Observe(time.Since(started).Seconds())
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		dbErrorsTotal.WithLabelValues("notification-service", operation).Inc()
	}
}
