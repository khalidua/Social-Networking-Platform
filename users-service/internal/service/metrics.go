...
func observeBusinessOperation(operation string, started time.Time, status string) {
	businessOperationDuration.WithLabelValues("users-service", operation).Observe(time.Since(started).Seconds())
	businessOperationTotal.WithLabelValues("users-service", operation, status).Inc()
}
